...
Twitter.prototype._checkBitly = function (auth) {
  var self = this;
  if (!auth) { 
    self.emit('badBitlyCreds', 'No bit.ly API key provided, disabling shortlinks to tweets.');
    self.bitly = false;
    return; 
  }
...
Twitter.prototype.startStream = function () {
  ...
  if (self.twitConn.activeStream) { 
    console.log('Stream already active.'.yellow);
    return; 
  }
...
Twitter.prototype.stopStream = function () {
  console.log('Ending streaming connection to Twitter...'.magenta);
...
Twitter.prototype.checkLanguage = function (data) {
  ...
  if ((results[0][0] !== 'english') && (results[0][0] !== 'pidgin')) {
    console.log('Likely Garbage: %s', data.text);
    console.log('Result Two: %j\nResult Three: %j');
    return;
  }
...
Twitter.prototype._initHook = function () {
  ...
  self.on('*::tweet', function (event, data) {
    self.sendTweet(data);
  });

  self.on('*::follow', function (event, data) {
    self.follow(data);
  });

  self.on('*::report', function (event, data) {
    self.reportSpam(data.name, data.to);
  });

  self.on('*::block', function (event, data) {
    self.block(data.name, data.to);
  });

  self.on('*::stopTweets', function (event, data) {
    self.stopStream();
  });

  self.on('*::startTweets', function (event, data) {
    self.startStream();
  })

  self.on('*::exit', function (event, data) {
    self.save(function (err) {
      if (err) {
        self.emit('error::saveError', err);
        process.exit(1);
      }
      process.exit(0);  
    });
